...
def get_proper_link(link):
    link_after = link.split(".zu.ac.ae")[-1]
    link_before = link.split(".zu.ac.ae")[0][12:]

    if "/main/" in link_after:
        return (link_before + link_after.split("/main")[-1]).replace("/", " ").replace("_", " ").replace(".", " ").strip()
    
    else:
        return (link_before + link_after).replace("/", " ").replace("_", " ").replace(".", " ").strip()
